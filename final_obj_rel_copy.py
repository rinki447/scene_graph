...
#from lib2.evaluation_recall import BasicSceneGraphEvaluator
...
class Config(object):
	...
	def setup_parser(self):
		...
		#parser.add_argument('-output_dir1', default=None, type=str)
...
class object_saver():
	def obj_save(original_video_path,data_path,modified_file_path,saving_dir_path,mode):
		...
		path= original_video_path #'/data/AmitRoyChowdhury/sayak/activity-net-captions/test_paths.txt' ###### main file
 
		path2= modified_file_path #'/data/AmitRoyChowdhury/sayak/activity-net-captions/test_paths_part1.txt' ##### to be changed: files from main file
		...
		for i in data_path_video:##################################### to be changed
			dir1.append(i)
		...
		#data_file_path=path2 #finally this is the file containing sorted data path to videos
		...
		#print("No of object classes:",len(COCO_dataset.object_classes))
		#model_checkpoint for faster RCNN for object detection
		#coco???? object_classes=AG_dataset.object_classes #----to b changed into coco
		...
		with torch.no_grad():
			...
			for b, data in enumerate(dataloader):
				#for b in range(1758,4000):
				#frames,vid_id,sampled_frame_ids = data[0],data[1],data[2]
				print(f"video {b} processing")
				#print("0th col of data",data[0].shape)
				#print("1st col of data",data[1].shape)
				#print("2nd col of data",data[2].shape)
				#print("3rd col of data",data[3].shape)
				v_id = copy.deepcopy(data[5])
				f_path = copy.deepcopy(data[6])
				path_list = copy.deepcopy(data[7])
				#list_f_path=copy.deepcopy(data[7])
				print("v_id:",v_id)
				...
				#print("f_path:",f_path)
				#print("*********************************************************************")
				#print("list f_path:",list_f_path)
				im_data = copy.deepcopy(data[0].cuda(0))
				im_info = copy.deepcopy(data[1].cuda(0))
				gt_boxes = copy.deepcopy(data[2].cuda(0))
				num_boxes = copy.deepcopy(data[3].cuda(0))
				gt_annotation = None #AG_dataset.gt_annotations[data[4]]
				#print(im_info)
				frame_no=im_data.shape[0]  #['bbox'].shape[0] == 0
				...
				#print("directory_path3",directory_path3)
				...
				# entry = clean_class(entry, b1, 5)
				# entry = clean_class(entry, b1, 8)
				# entry = clean_class(entry, b1, 17)
				...
				# can check if score for any box is below certain number???????????????????????????????????????????????????/
...
class annotation():
		def gt_annotation(saved_json_path,key_vid,entry,frame_box):
			...
			#print("gt annotation uniq frames",uniq_frames)
		
	   
			gt_annotation_video = []
			entry_frame={}
			#global_id=np.arange(0,entry["boxes"].shape[0])
			
			for i in uniq_frames:####### each frame j

				ind=torch.nonzero(entry["boxes"][:,0]==i).view(1,-1)
				ind=ind.squeeze(0)
				#global_hid1=global_id[ind]
