...
from lib.word_vectors_2 import obj_edge_vectors
from lib.transformer import transformer
from lib.fpn.box_utils import center_size
from lib.gmm_heads import GMM_head
...
class ObjectClassifier(nn.Module):
    ...
    def forward(self, entry, phase='train', unc=False):
        ...
        if self.mode  == 'predcls':
            entry['pred_labels'] = entry['labels']
            #dict_keys(['boxes', 'labels', 'scores', 'im_idx', 'pair_idx', 'human_idx', 'features', 'union_feat', 'union_box', 'spatial_masks', 'pred_labels'])
            ...
            if phase == 'train':
                ...
                entry['pred_labels'][HUMAN_IDX.squeeze()] = 80
                entry['pred_scores'][HUMAN_IDX.squeeze()] = entry['distribution'][HUMAN_IDX.squeeze(), 0]
                ...
                for j, i in enumerate(HUMAN_IDX):
                    for m in global_idx[box_idx == j][
                        entry['pred_labels'][box_idx == j] != 80]:  # this long term contains the objects in the frame
                        im_idx.append(j)
                        pair.append([int(i), int(m)])
...
class TEMPURA(nn.Module):
    ...
    def forward(self, entry, phase='train',unc=False):
        ...
        #a,b=np.unique(entry['pair_idx'][:, 0].cpu().numpy(),return_counts=True)
        #print("unique human idx",a,b)
        #exit()

        # visual part
        if not self.take_obj_mem_feat:
            print("only features taken")
            subj_rep = entry['features'][entry['pair_idx'][:, 0]].to(device=torch.device('cuda:0'))
            obj_rep = entry['features'][entry['pair_idx'][:, 1]].to(device=torch.device('cuda:0'))
        else:
            print("memory feature taken")
            subj_rep = entry['object_mem_features'][entry['pair_idx'][:, 0]].to(device=torch.device('cuda:0'))
            obj_rep = entry['object_mem_features'][entry['pair_idx'][:, 1]].to(device=torch.device('cuda:0'))


        subj_rep = self.subj_fc(subj_rep)       
        obj_rep = self.obj_fc(obj_rep)
        print(subj_rep.shape)
        print(obj_rep.shape)
        ...
        #print("vr",vr.cpu().detach().numpy().tolist())
        #exit()
        ...
        # semantic part
        #print(entry['pred_labels'][entry['pair_idx'][:, 0]])
        subj_class = entry['pred_labels'][entry['pair_idx'][:, 0]]+1
        obj_class = entry['pred_labels'][entry['pair_idx'][:, 1]]+1
        
        subj_class = subj_class.to(torch.int64).to(device=torch.device('cuda:0'))
        obj_class = obj_class.to(torch.int64).to(device=torch.device('cuda:0'))


        subj_emb = self.obj_embed(subj_class)
        obj_emb = self.obj_embed2(obj_class)
        #print(subj_emb.shape)
        #print(obj_emb.shape)
        
        # s1=subj_emb.cpu().detach().numpy()
        # np.save('subj_tmp2.npy',s1)

        # o1=obj_emb.cpu().detach().numpy()
        # np.save('obj_tmp2.npy',o1)
        
        #print("obj_cls1",obj_class)
